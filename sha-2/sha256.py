...
# SHA-2 additional functions
def parseUint32(a):
    return cu.bytes_to_long(a) & 0xFFFFFFFF


def rotateRight(w, _n):
    return (((w & 0xffffffff) >> (_n & 31)) | (w << (32 - (_n & 31)))) & 0xffffffff
...
def pad(_m, _n):
    _tmp = (len(_m) * 8) & 0xFFFFFFFFFFFFFFFF
    _m.append(0x80)

    while (len(_m) % _n) != (_n - 8):
        _m.append(0x00)

    _m = (cu.bytes_to_long(_m) << 64) | _tmp

    mBytes = cu.long_to_bytes(_m)
...
print(hex(sha256(bytearray(input("Write:").encode()))))